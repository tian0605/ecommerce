...
## 2026-03-28 系统性Skill改造原则

### 决策
**用户确认：** 在 `__init__.py` 导出 `run()` 函数，统一返回格式
...
### 实施步骤
1. miaoshou-collector: 添加 run() 函数
2. collector-scraper: 添加 run() 函数
3. miaoshou-updater: 添加 run() 函数
4. local-1688-weight: 已有函数式接口，可直接用
5. product-storer: 已有函数式接口
6. listing-optimizer: 添加 run() 函数

### workflow_executor 简化
```python
# 简化后只需要：
...
### 时机
- 当前：继续测试 AUTO-LISTING-004 验证流程
- 之后：系统性改造现有skill，添加统一run()接口

## 2026-03-28 浏览器自动化改进方案

### 问题分类
| 问题 | 根因 | 解决方案 |
|------|------|----------|
| 找不到元素 | 选择器过时/页面变化 | AI视觉分析+截图 |
| 弹窗没清理 | close_popups()时机不对 | 每次操作前清理 |
| 页面没加载 | 太快操作DOM | 等待networkidle |
| Vue点击无效 | 需要JS模拟 | 使用evaluate() |
...
class SafeBrowser:
    ...
    def close_all_popups(self):
        # 关闭所有弹窗
        selectors = ['.el-dialog__wrapper', '.el-message-box__wrapper', 
...
### 改进2: 失败时AI分析
```python
def analyze_error_state(page, context):
    """失败时截图并用AI分析"""
...
### 改进3: 增强型截图
```python
def take_debug_screenshot(page, name, analysis_prompt=None):
    """带AI分析的截图"""
    page.screenshot(path=f'/tmp/{name}.png')
    if analysis_prompt:
        img = Image.open(f'/tmp/{name}.png')
        result = vision_model.analyze(img, analysis_prompt)
        return result
```
