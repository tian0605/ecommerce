...
logger = setup_logger('fix-step6-v2')
...
def click_publish_dialog(updater):
    """尝试找到并点击发布确认对话框中的确定按钮"""
    # 方法1：使用Playwright原生query
    try:
        all_dialogs = updater.page.query_selector_all('.el-dialog__wrapper')
        for dlg in all_dialogs:
            try:
                title_elem = dlg.query_selector('.el-dialog__title')
                if title_elem and '发布产品' in title_elem.inner_text():
                    logger.info("  方法1: 找到发布对话框")
                    # 勾选checkbox
                    cbs = dlg.query_selector_all('input[type=checkbox]')
                    for cb in cbs:
                        try:
                            if not cb.is_checked():
                                cb.check()
                        except: pass
                    logger.info(f"  已勾选checkbox: {len(cbs)}个")
                    # 找确定发布按钮
                    ...
                    for btn in btns:
                        ...
                        if '确定发布' in txt:
                            ...
                            logger.info("  点击确定发布成功!")
                            time.sleep(10)
                            return True
                    # 方法2: JS直接点击
                    logger.info("  方法1未找到按钮，尝试JS点击...")
                    ...
                    """)
                    logger.info(f"  JS点击结果: {result}")
                    if result == 'clicked':
                        time.sleep(10)
                        return True
            except Exception as e:
                logger.debug(f"  处理对话框异常: {e}")
    except Exception as e:
        ...
        result = updater.page.evaluate("""
            () => {
                ...
                for (var btn of buttons) {
                    if (btn.innerText && btn.innerText.trim().includes('确定发布')) {
                        btn.click();
                        return 'clicked';
                    }
                }
...
def process_product(updater, product):
    ...
    time.sleep(10)
    updater.screenshot(f'edit_{alibaba_id}_1')
    ...
    updater.page.evaluate(("""
        ...
        if (nodes.length >= 3) {
            ...
            for (var n of nodes[0].querySelectorAll(".el-cascader-node")) {
                if ((n.innerText || "").indexOf("%s") >= 0) { n.click(); found = true; break; }
            }
        }
    """ % level1))
    time.sleep(0.6)
    ...
    for i in range(20):
        time.sleep(1)
        dialogs = updater.page.query_selector_all('.el-dialog__wrapper')
        for dlg in dialogs:
            try:
                title = dlg.query_selector('.el-dialog__title')
                if title and '发布产品' in title.inner_text():
                    dialog_appeared = True
                    logger.info(f"  发布对话框已出现!")
                    break
            except: pass
        if dialog_appeared:
            break
        logger.info(f"    等待发布对话框... ({i+1}/20)")
    ...
    # 7. 处理发布对话框
    success = click_publish_dialog(updater)
    ...
    if success:
        ...
        conn.commit()
        conn.close()
        logger.info(f"  ✅ 发布成功，数据库已更新为published: {alibaba_id}")
        return True
    else:
        logger.warning(f"  ⚠️ 发布对话框处理失败: {alibaba_id}")
        return False
